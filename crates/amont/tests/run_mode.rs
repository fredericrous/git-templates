//! `amont run` — the checks, on demand.
//!
...
/// path) and `amont run` failed — the two modes disagreed about the same
...
/// `--all-files` with `amont.fix` on must stage NOTHING.
///
...
/// `amont/tests/common::Repo::git` always closes the child's stdin).
#[test]
