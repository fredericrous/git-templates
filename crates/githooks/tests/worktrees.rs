...
/// Run `githooks pre-commit` exactly the way the real shim would from a
...
fn hook_from_worktree(
    ...
    Repo::strip_git_env_impl(&mut cmd);
    let out = cmd.output().expect("run githooks pre-commit");
    (
...
fn unstaged_work_in_a_worktree_survives_the_stash_and_restore() {
    ...
    assert!(
        !worktree_git_dir(&wt).join("githooks-held").exists(),
