...
/// Whether a stray `githooks` on `MINIMAL_PATH` would make these tests vacuous.
fn githooks_on_minimal_path() -> bool {
    MINIMAL_PATH
        .split(':')
        .any(|d| Path::new(d).join("githooks").exists())
}

/// Run the hook the way git does: through `sh`, from the worktree root, with an
/// environment that resolves no githooks binary by itself.
///
...
fn an_unbaked_shim_ignores_an_executable_the_repo_planted() {
    // A githooks on the minimal PATH would resolve candidate 4 and make the
    // assertion below vacuous rather than wrong.
    if githooks_on_minimal_path() {
        return;
    }
    let r = Repo::new();
    ...
    let payload = r.dir.join("__GITHOOKS_BIN__");
    std::fs::write(
        &payload,
        format!("#!/bin/sh\ntouch {}\nexit 0\n", sentinel.display()),
    )
    ...
    assert!(
        err.contains("githooks binary not found"),
...
/// `.githooks.conf`, no prompt, no declaration.
#[test]
...
fn an_unbaked_shim_ignores_a_directory_the_repo_planted() {
    ...
    std::fs::create_dir_all(r.dir.join("__GITHOOKS_BIN__")).expect("plant dir");
    ...
    assert!(
        String::from_utf8_lossy(&out.stderr).contains("githooks binary not found"),
...
fn a_baked_absolute_path_still_wins() {
    ...
    let fake = empty_dir("bin").join("githooks");
    std::fs::write(
        &fake,
        format!(
            "#!/bin/sh\nprintf '%s\\n' \"$@\" > {}\nexit 0\n",
    ...
    let baked = template().replace("__GITHOOKS_BIN__", &fake.display().to_string());
...
fn a_relative_baked_path_is_refused_even_when_it_resolves() {
    ...
    let payload = r.dir.join("tools").join("githooks");
    std::fs::write(
        &payload,
        format!("#!/bin/sh\ntouch {}\nexit 0\n", sentinel.display()),
    )
    ...
    let baked = template().replace("__GITHOOKS_BIN__", "tools/githooks");
    let hook = install_hook(&r, &baked);
    let out = run_hook(&r, &hook, &home);
