...
fn each_placement_puts_the_emoji_where_it_says() {
    for (placement, expected) in [
        ...
        let r = Repo::new();
        r.git(&["config", "githooks.commit.gitmoji", placement]);
        let (ok, out) = rewrite(&r, "feat: add a thing\n");
...
fn the_configured_limits_are_what_is_enforced() {
    ...
    r.git(&["config", "githooks.commit.descriptionMax", "60"]);
    ...
    let r2 = Repo::new();
    r2.git(&["config", "githooks.commit.subjectMax", "20"]);
...
fn a_zero_wrap_column_leaves_the_body_alone() {
    ...
    let r2 = Repo::new();
    r2.git(&["config", "githooks.commit.bodyWrap", "0"]);
...
fn a_bad_config_value_is_never_silent() {
    let r = Repo::new();
    r.git(&["config", "githooks.commit.descriptionMax", "wide"]);
    r.write("MSG", "feat: a short one\n");
    let run = r.hook("commit-msg", &["MSG"]);
    assert!(run.passed(), "the default should still apply");
    assert!(
        run.says("githooks.commit.descriptionMax"),
...
fn a_limit_outside_its_range_is_reported_and_ignored() {
    let r = Repo::new();
    r.git(&["config", "githooks.commit.subjectMax", "0"]);
    r.write("MSG", "feat: a short one\n");
    let run = r.hook("commit-msg", &["MSG"]);
    assert!(run.passed(), "a zero limit must not block everything");
    assert!(run.says("githooks.commit.subjectMax"), "{:?}", run.output());
}
...
fn a_placement_is_matched_however_it_is_spelled() {
    for spelling in ["prefix", "PREFIX", "Prefix"] {
        let r = Repo::new();
        r.git(&["config", "githooks.commit.gitmoji", spelling]);
        let (ok, out) = rewrite(&r, "feat: add a thing\n");
        assert!(ok);
        assert_eq!(
            out.lines().next(),
...
fn an_unknown_placement_lists_the_ones_that_exist() {
    let r = Repo::new();
    r.git(&["config", "githooks.commit.gitmoji", "sideways"]);
    r.write("MSG", "feat: a short one\n");
    let run = r.hook("commit-msg", &["MSG"]);
    assert!(run.passed());
    let said = run.output();
