...
impl Gitmoji {
    ...
    /// The one-line description `amont setup` and `amont list` show, so
...
///
/// `commit-msg` writes the real thing and `amont setup` renders the menu
...
mod tests {
    ...
    /// that word is what `git config` stores and `amont setup` writes.
    #[test]
