//! What `githooks-fleet` is willing to write over, driven through the real
//! binary.
//!
...
fn fake_binary(t: &Tree) -> PathBuf {
    let b = t.path().join("fake-githooks");
...
fn apply_never_writes_through_a_symlinked_dispatcher() {
    ...
    let all = repo_with_symlinked_dispatchers(&t, "all", "/opt/other-githooks", &DISPATCHERS);
    ...
    let one = repo_with_symlinked_dispatchers(&t, "one", "/opt/other-githooks", &["commit-msg"]);
