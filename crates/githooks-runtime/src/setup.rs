//! `githooks setup` — choose what `commit-msg` enforces, once.
//!
...
pub fn command(args: &[std::ffi::OsString]) -> Result<(), String> {
    ...
    if asked_local && asked_global {
        return Err("githooks setup: --local and --global contradict each other".to_string());
    }
    if let Some(bad) = args.iter().find(|a| {
        ...
        return Err(format!(
            "githooks setup: unknown argument {:?}\nusage: githooks setup [--local|--global] [--dry-run]",
            bad.to_string_lossy()
        ));
    }
    ...
    if asked_local && git::stdout(&["rev-parse", "--show-toplevel"]).is_none() {
        return Err("githooks setup --local: not inside a git repository".to_string());
    }
...
fn ask_all(
    ...
    println!("githooks setup — what `commit-msg` enforces, and how it decorates.");
...
fn apply(answers: &[Answer], scope: Where, dry_run: bool) -> Result<(), String> {
    ...
    if changed.is_empty() {
        ...
        if !dry_run {
            for a in &changed {
                ...
                if !ok {
                    return Err(format!("githooks setup: could not write {}", a.key));
                }
            }
        }
    }
    ...
    println!("  git config githooks.fix true             # let a check fix what it finds");
    println!("  git config githooks.testPushedTree true  # test what you push, not your tree");
    println!("\nRead it all back with:  githooks list");
    Ok(())
}
...
fn offer_the_commands(style: &Style, scope: Where) -> Result<(), String> {
    eprintln!(
        "githooks setup: not a terminal — nothing to ask. The keys, with their current values:"
    );
    println!(
