//! `githooks agents-md` — a self-verifying pointer for coding agents.
//!
...
//! added, removed, or its severity changes — see `githooks-fleet`'s
...
//! `githooks list --json`, which cannot go stale because it IS the registry.
//!
...
//! rewrite — matching `.githooks.conf`'s "declared, never assumed" posture.

use std::ops::Range;
use std::path::Path;

pub const START: &str = "<!-- githooks:start -->";
pub const END: &str = "<!-- githooks:end -->";
...
```sh\n\
githooks list --json\n\
githooks list --json --stage pre-push --pushed  # exactly what pushing next gates\n\
```\n\
\n\
...
pub fn write(path: &Path) -> Result<(), String> {
    ...
    let content = desired_file_content(&existing).map_err(|_| {
        format!(
            "{}: has an unpaired githooks marker — fix or remove it by hand, \
             then re-run `githooks agents-md`",
            path.display()
        )
    })?;
...
mod tests {
    ...
    fn check_reports_not_present_for_a_missing_file() {
        let tmp = std::env::temp_dir().join("githooks-agents-md-test-nonexistent-xyz");
