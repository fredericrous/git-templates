...
#     cp target/release/githooks /tmp/shop-home/.local/bin/githooks
...
#     printf '{"items": []}\n'            > data.json
#     git add -A
#     /tmp/shop-home/.local/bin/githooks install < /dev/null > /dev/null
#     DEMO_BIN=/tmp/shop-home/.local/bin/githooks asciinema rec --overwrite \
#         --cols 96 --rows 30 --command "sh <repo>/docs/assets/demo.sh" \
#         <repo>/docs/assets/githooks-demo.cast
#     agg --theme monokai --font-size 15 --idle-time-limit 2 \
#         <repo>/docs/assets/githooks-demo.cast \
#         <repo>/docs/assets/githooks-demo.gif
#
...
# strangers.
#
# `githooks install` runs BEFORE the recording rather than inside it: it prompts
# about AGENTS.md on a tty, and a demo that hangs waiting for a keypress records
# the hang.
set -eu

BIN="$DEMO_BIN"
export GITHOOKS_BIN="$BIN"
...
say 'githooks list --stage pre-push'
