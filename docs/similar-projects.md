...
worth your time. This page is what you get by choosing githooks instead — and
what you give up.

## At a glance

| | githooks | pre-commit | lefthook | husky |
|---|---|---|---|---|
...
tool's YAML. githooks ships the decision — conventional commit subjects, no
...
githooks binary links no external crates — `std` only, with a CI script that
...
`npm install`, a `postinstall`, an unreviewed `pre-commit install` — githooks
holds a repository's declared checks **inert until you have read them and said
[`githooks trust`](trust.md)**. Nobody else on this list has that gate.
...
`hook.skip` was set three years ago and forgotten? githooks has a designed
...
- **pre-commit's ecosystem is enormous.** Hundreds of ready-made hooks, and it
  bootstraps each hook's language environment for you. githooks deliberately
...
- **lefthook's config is very expressive** — glob routing, piped commands,
  scripts, tags, per-OS overrides — where [`.githooks.conf`](custom-checks.md)
...
- **From pre-commit:** a repository declaring its own checks in a committed
  ...
  installing it by hand — taken as [`.githooks.conf`](custom-checks.md), with
